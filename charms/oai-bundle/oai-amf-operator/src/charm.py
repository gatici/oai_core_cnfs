...
class OaiAmfCharm(CharmBase):
    ...
    def __init__(self, *args):
        ...
        # Action hooks
        self.framework.observe(self.on.starttcpdump_action, self._on_starttcpdump_action)
        self.framework.observe(self.on.stoptcpdump_action, self._on_stoptcpdump_action)

    ####################################
    # Custom Actions
    ####################################

    def _on_starttcpdump_action(self, event):
        try:
            self._start_service("tcpdump", "tcpdump")
            event.set_results({
                    "output": f"tcpdump started successfully"
                    })
        except Exception as e:
            event.fail(f"Start tcpdump action failed with the following exception: {e}")

    def _on_stoptcpdump_action(self, event):
        try:
            self._stop_service("tcpdump", "tcpdump")
            event.set_results({
                    "output": f"tcpdump stoped successfully"
                    })
        except Exception as e:
            event.fail(f"Stop tcpdump action failed with the following exception: {e}")
